...
- `pbir_set_report` connects to the `.Report` folder on disk
- `pbir_list_pages` returns every page with its id, name, visual count, and hidden status
...
- `pbir_create_page` creates a page named "Quick Stats" (defaults to 1280x720)
- `pbir_add_visual` adds a card visual bound to `financials[Gross Sales]` with aggregation Sum
...
- `pbir_list_pages` finds the page id for "Quick Stats"
- `pbir_add_visual` creates a slicer visual at position x=1070, y=10, width=200, height=50 with mode "Dropdown" bound to `financials[Year]` as a column
...
- `pbir_create_page` creates the page at 1280x720
- `pbir_add_visual` (batch) creates all visuals in a single call:
...
- Total: 2 tool calls (pbir_create_page + one batched pbir_add_visual)
...
- `pbir_list_pages` resolves the page id
- `pbir_add_visual` (batch) adds two visuals:
...
- `pbir_set_report_theme` writes a custom theme JSON with the specified dataColors array, background, foreground, and tableAccent
...
- `pbir_list_visuals` finds the bar chart visual id on the target page
- `pbir_format_visual` applies multiple formatting properties in one call:
...
### 13. Rebind visuals to different measures using pbir_bulk_bind
...
- `pbir_list_visuals` gets the visual ids for the three cards
- `pbir_bulk_bind` rebinds all three visuals in a single call, changing each card's Values bucket from aggregation "Sum" to aggregation "Avg" on their respective fields (`financials[Gross Sales]`, `financials[Profit]`, `financials[Units Sold]`)
- Total: 2 tool calls instead of 3 separate pbir_update_visual_bindings calls
...
1. `pbir_set_report` -- connect to the target .Report folder
2. `pbir_set_report_theme` -- apply the corporate blue theme globally (dataColors, background, foreground, tableAccent)
3. `pbir_create_page` x5 -- create all five pages ("Executive Summary", "Sales by Country", "Product Analysis", "Time Trends", "Segment Breakdown")
4. `pbir_add_visual` (batch) x5 -- one batch call per page, each containing:
...
- **Batch when you can.** A single prompt that describes an entire page will generate one batch `pbir_add_visual` call instead of many individual ones.
