...
/**
 * Standalone usage dashboard app.
 * Double-click launch-usage.bat → opens browser → pick report → see dashboard.
 * No MCP, no terminal interaction needed.
 */
...
// ---------------------------------------------------------------------------
const RECENTS_FILE = path.join(process.env.LOCALAPPDATA || process.env.HOME || ".", "powerbi-report-mcp", "recent-reports.json");
...
function saveRecent(reportPath) {
...
// Landing page HTML
// ---------------------------------------------------------------------------
function landingHTML(recents, error) {
...
<title>Power BI Usage Dashboard</title>
<style>
...
function parseQuery(url) {
    ...
    url.slice(idx + 1).split("&").forEach(p => {
        const [k, v] = p.split("=");
        if (k)
            q[decodeURIComponent(k.replace(/\+/g, " "))] = decodeURIComponent((v || "").replace(/\+/g, " "));
    });
    return q;
}
const server = http.createServer((req, res) => {
    ...
    if (pathname === "/pick-folder") {
        ...
        if (process.platform === "win32") {
            ...
            exec(`powershell -NoProfile -Command "${ps}"`, { timeout: 120000 }, (err, stdout) => {
    ...
    if (pathname === "/generate") {
        ...
        try {
            ...
            res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
            res.end(landingHTML(loadRecents(), e.message));
            return;
        }
        try {
            ...
            res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
            res.end(landingHTML(loadRecents(), `Error generating dashboard: ${e.message}`));
        }
        return;
    }
    res.writeHead(404);
