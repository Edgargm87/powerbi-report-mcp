<!-- mirrored from skills/elicitation.md at v0.9.6 (08eda17) -->
...
| **Small build** — 2–3 visuals on an existing page | "Add a KPI strip to page 2", "Put a trend chart next to the card" | **Zero.** Pick sensible defaults from `pbir_model_usage`, build. Show result. |
...
| **Blank report, no prompt context** | User connects via `pbir_set_report` and says "what can you do?" | Offer the menu. Don't auto-build. |

Four rules that keep this honest:

1. **Count visuals in your head before you ask anything.** If the answer is ≤3, don't ask.
2. **Modifications never trigger elicitation.** If `pbir_list_pages` shows existing content and the user's request is a tweak, build it. The existing page is the spec.
...
If the user's prompt contains any of these, go straight to building. No questions, no confirmations before the first `pbir_add_visual`:
...
Once the user has answered any elicitation question in a session, **those answers stick for the rest of the session**. Subsequent page builds don't re-ask — they reuse the answers silently. The only reset is a new `pbir_set_report` to a different report path.
...
If a `report-spec.md` file sits next to the `.pbip`, it pre-populates all 5 signals. The agent reads it on `pbir_set_report` and skips elicitation entirely unless the user's request contradicts it. Minimal shape:
...
|---|---|---|---|
| **1–3 visuals** | skip | skip | optional — offer a `pbir_reload_report` and stop |
| **4–7 visuals** | **yes** — one message, confirm plan | skip | optional |
| **8+ visuals or multi-page** | **yes** | **yes** — `pbir_list_pages({includeVisuals: true})`, confirm before binding | **yes** — confirm after binding, before polish |
